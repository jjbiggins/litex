...
class EfinixDbParser:
    def __init__(self, efinity_path, device):
        self.efinity_db_path = f'{efinity_path}/pt/db/'
        self.device = device

    def get_device_map(self, device):
        with open(f'{self.efinity_db_path}devicemap.csv') as f:
            reader = csv.reader(f)
            data = list(reader)

        return next((d for d in data if d[0] == device), None)
    ...
    def get_block_instance_names(self, block):
        ...
        names = [p.get('name') for p in peri if p.get('block') == block]
    ...
    def get_pll_inst_from_gpio_inst(self, dmap, inst):
        ...
        for p in peri:
            ...
            if p.get('block') == 'pll':
                if self.device[:2] not in ['T4', 'T8']:
                    conn = p.findall('efxpt:single_conn', namespaces)
                    for c in conn:
                        i = c.get('instance')
                        if i is None:
                            continue
                        if i == inst or f'{inst}.' in i:
                            refclk_no = 0
                            if c.get('index') == '3':
                                refclk_no = 1
