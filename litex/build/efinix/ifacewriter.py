...
class InterfaceWriter:
    ...
    def fix_xml_values(self):
        ...
        tree = et.parse(f"{self.filename}.peri.xml")
        root = tree.getroot()
        ...
        tools.write_to_file(f"{self.filename}.peri.xml", print_string)
    ...
    def header(self, build_name, partnumber):
        header = f"# Autogenerated by LiteX / git: {tools.get_litex_git_revision()}"
        header += """
import os
import sys
import pprint
...
"""
    ...
    def generate_mipi_rx(self, block, verbose=True):
        ...
        cmd = f"# ---------- MIPI RX {name} ---------\n"
        cmd += (
            f'design.create_block("{name}","MIPI_RX_LANE", mode="{block["mode"]}"{conn_type}'
            + ')\n'
        )
    ...
    def generate_gpio(self, block, verbose=True):
        ...
        if mode == "INOUT":
            ...
            if prop:
                for p, val in prop:
                    cmd += f'design.set_property("{name}","{p}","{val}")\n'
            cmd += "\n"
            return cmd
        ...
        if mode == "OUTPUT":
            if len(block["location"]) == 1:
                cmd += f'design.create_output_gpio("{name}")\n'
                cmd += f'design.assign_pkg_pin("{name}","{block["location"][0]}")\n'
            else:
                cmd += f'design.create_input_gpio("{name}",{block["size"] - 1},0)\n'
                for i, pad in enumerate(block["location"]):
            ...
            if "drive_strength" in block:
                cmd += f'design.set_property("{name}","DRIVE_STRENGTH","4")\n'
        ...
        if mode == "INPUT_CLK":
            cmd += f'design.create_input_clock_gpio("{name}")\n'
            cmd += f'design.set_property("{name}","IN_PIN","{name}")\n'
            cmd += f'design.assign_pkg_pin("{name}","{block["location"]}")\n\n'
            if prop:
                for p, val in prop:
                    cmd += f'design.set_property("{name}","{p}","{val}")\n'
            cmd += "\n"
        ...
        if mode == "MIPI_CLKIN":
            cmd += f'design.create_mipi_input_clock_gpio("{name}")\n'
            cmd += f'design.assign_pkg_pin("{name}","{block["location"]}")\n\n'
            return cmd

        if mode == "OUTPUT_CLK":
            cmd += f'design.create_clockout_gpio("{name}")\n'
            cmd += f'design.set_property("{name}","OUT_CLK_PIN","{name}")\n'
            cmd += f'design.assign_pkg_pin("{name}","{block["location"]}")\n\n'
            if prop:
                for p, val in prop:
                    cmd += f'design.set_property("{name}","{p}","{val}")\n'
    ...
    def generate_pll(self, block, partnumber, verbose=True):
        ...
        cmd = (
            f"# ---------- PLL {name} ---------\n"
            + f'design.create_block("{name}", block_type="PLL")\n'
        )

        cmd += 'pll_config = {{ "REFCLK_FREQ":"{}" }}\n'.format(block["input_freq"] / 1e6)
        cmd += f'design.set_property("{name}", pll_config, block_type="PLL")\n\n'

        if block["input_clock"] == "EXTERNAL":
            # PLL V1 has a different configuration
            if partnumber[:2] in ["T4", "T8"]:
                cmd += f'design.gen_pll_ref_clock("{name}", pll_res="{block["resource"]}", refclk_res="{block["input_clock_pad"]}", refclk_name="{block["input_clock_name"]}", ext_refclk_no="{block["clock_no"]}")\n\n'

            else:
                cmd += f'design.gen_pll_ref_clock("{name}", pll_res="{block["resource"]}", refclk_src="{block["input_clock"]}", refclk_name="{block["input_clock_name"]}", ext_refclk_no="{block["clock_no"]}")\n\n'

        else:
            cmd += f'design.gen_pll_ref_clock("{name}", pll_res="{block["resource"]}", refclk_name="{block["input_signal"]}", refclk_src="CORE")\n'

            cmd += f'design.set_property("{name}", "CORE_CLK_PIN", "{block["input_signal"]}", block_type="PLL")\n\n'


        cmd += f'design.set_property("{name}","LOCKED_PIN","{block["locked"]}", block_type="PLL")\n'

        if block["rstn"] != "":
            cmd += f'design.set_property("{name}","RSTN_PIN","{block["rstn"]}", block_type="PLL")\n\n'
        ...
        for i, clock in enumerate(block["clk_out"]):
            if block["version"] == "V1_V2":
                cmd += f'design.set_property("{name}","CLKOUT{i}_PHASE","{clock[2]}","PLL")\n'
            else:
                cmd += f'design.set_property("{name}","CLKOUT{i}_PHASE_SETTING","{clock[2] // 45}","PLL")\n'


        cmd += "target_freq = {\n"
        for i, clock in enumerate(block["clk_out"]):
            cmd += f'    "CLKOUT{i}_FREQ": "{clock[1] / 1e6}",\n'
        cmd += "}\n"
        cmd += f'calc_result = design.auto_calc_pll_clock("{name}", target_freq)\n'
...
design.save()"""
    ...
    def add_lvds_xml(self, root, params):
        ...
        et.SubElement(
            ...
            "efxpt:ltx_info",
            pll_instance="",
            fast_clock_name=f'{params["fast_clk"]}',
            slow_clock_name=f'{params["slow_clk"]}',
            reset_name="",
            out_bname=f'{params["name"]}',
            oe_name="",
            clock_div="1",
            mode=f"{mode}",
            serialization=f'{params["serialisation"]}',
            reduced_swing="false",
            load="3",
        )
