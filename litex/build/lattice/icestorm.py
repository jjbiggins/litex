    ...
    for sig, pins, others, resname in named_sc:
        if len(pins) > 1:
            for bit, pin in enumerate(pins):
                r += f"set_io {sig}[{bit}] {pin}\n"
        else:
            r += f"set_io {sig} {pins[0]}\n"
    if named_pc:
        r += "\n" + "\n\n".join(named_pc)
    return r
...
def _build_pre_pack(vns, clocks):
    return "".join(
        f"""ctx.addClock("{vns.get_name(clk)}", {1e3 / period})\n"""
...
def parse_device(device):
    ...
    if architecture not in packages.keys():
        raise ValueError(f"Invalid device architecture {architecture}")
    if package not in packages[architecture]:
        raise ValueError(f"Invalid device package {package}")
...
class LatticeIceStormToolchain:
    ...
    def build(self, platform, fragment,
        ...
        **kwargs):
        ...
        tools.write_to_file(f"{build_name}.pcf", _build_pcf(named_sc, named_pc))

        # Generate design timing constraints file (in pre_pack file)
        tools.write_to_file(
            f"{build_name}_pre_pack.py", _build_pre_pack(v_output.ns, self.clocks)
        )
