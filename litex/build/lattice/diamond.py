...
def _build_lpf(named_sc, named_pc, clocks, vns, build_name):
    ...
    for clk, period in clocks.items():
        ...
        lpf.append(
            f'FREQUENCY {"PORT" if clk_name in [name for name, _, _, _ in named_sc] else "NET"} \"{clk_name}\" {str(1e3/period)} MHz;'
        )
...
def _build_script(build_name, device):
    ...
    for ext in (".bit", ".jed"):
        ...
        script_contents += "{copy_stmt} {diamond_product} {migen_product} {fail_stmt}\n".format(
            copy_stmt=copy_stmt,
            fail_stmt=fail_stmt,
            diamond_product=os.path.join("impl", f"{build_name}_impl{ext}"),
...
def _run_script(script):
    on_windows = sys.platform in ("win32", "cygwin")
    shell = ["cmd", "/c"] if on_windows else ["bash"]
    if which("pnmainc" if on_windows else "diamondc") is None:
        msg = (
            "Unable to find Diamond toolchain, please:\n"
            + "- Add Diamond toolchain to your $PATH.\n"
        )
