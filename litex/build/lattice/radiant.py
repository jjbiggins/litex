...
""".format(build_name=build_name)
    ...
    if which("yosys") is None:
        msg = (
            "Unable to find Yosys toolchain, please:\n"
            + "- Add Yosys toolchain to your $PATH."
        )
...
def _format_ldc(signame, pin, others, resname):
    ...
    ldc = [pre + signame + suf for pre, suf in fmt_c]
    return "\n".join(ldc)
...
def _build_pdc(named_sc, named_pc, clocks, vns, build_name):
    ...
    for clk, period in clocks.items():
        ...
        pdc.append(
            f'create_clock -period {str(period)} -name {clk_name} [{"get_ports" if clk_name in [name for name, _, _, _ in named_sc] else "get_nets"} {clk_name}];'
        )


    tools.write_to_file(f"{build_name}.pdc", "\n".join(pdc))
...
def _build_script(build_name, device):
    ...
    script_contents += "{tool} {tcl_script}{fail_stmt}\n".format(
        tool=tool, tcl_script=f"{build_name}.tcl", fail_stmt=fail_stmt
    )


    script_contents += (
        "{copy_stmt} {radiant_product} {migen_product} {fail_stmt}\n".format(
            copy_stmt=copy_stmt,
            fail_stmt=fail_stmt,
            radiant_product=os.path.join("impl", f"{build_name}_impl.bit"),
            migen_product=f"{build_name}.bit",
        )
    )


    build_script_file = f"build_{build_name}{script_ext}"
...
def _run_script(script):
    ...
    if which(tool) is None:
        msg = (
            "Unable to find Radiant toolchain, please:\n"
            + "- Add Radiant toolchain to your $PATH."
        )
...
def _check_timing(build_name):
    lines = open(f"impl/{build_name}_impl.par", "r").readlines()
    runs = [None, None]
