    ...
    def generate(self):
        ...
        for entry in self.ast:
            if isinstance(entry, str):
                ...
                makefile.extend(f"\t{cmd}" for cmd in entry.commands)
        return "\n".join(makefile)
...
def _run_make():
    ...
    if which("nextpnr-xilinx") is None:
        msg = (
            ...
            + "- Add Yosys and Nextpnr tools to your $PATH."
        )
    ...
    def _check_properties(self, platform):
        if not self.symbiflow_device:
            try:
                ...
                raise ValueError("symbiflow_device is not specified")
    ...
    def _generate_makefile(self, platform, build_name):
        ...
        makefile = _MakefileGenerator(
            [
                ...
                "",
                Var("DB_DIR", "/usr/share/nextpnr/prjxray-db"),
                Var("CHIPDB_DIR", "/usr/share/nextpnr/xilinx-chipdb"),
                "",
                Var(
                    "VERILOG",
                    [
                        f
                        for f, language, _ in platform.sources
                        if language in ["verilog", "system_verilog"]
                    ],
                ),
                Var(
                    "MEM_INIT",
                    [f"{name}" for name in os.listdir() if name.endswith(".init")],
                ),
