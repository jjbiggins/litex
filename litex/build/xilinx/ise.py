...
def _format_constraint(c):
    if isinstance(c, Pins):
        return f"LOC={c.identifiers[0]}"
    elif isinstance(c, IOStandard):
        return f"IOSTANDARD={c.name}"
    elif isinstance(c, Drive):
        return f"DRIVE={str(c.strength)}"
    ...
    for sig, pins, others, resname in named_sc:
        if len(pins) > 1:
            for i, p in enumerate(pins):
                r += _format_ucf(f"{sig}({str(i)})", p, others, resname)
        else:
...
def _build_xst(device, sources, vincpaths, build_name, xst_opt):
    prj_contents = "".join(
        f"{language} {library} {tools.cygpath(filename)}" + "\n"
        for filename, language, library, *copy in sources
    )

    tools.write_to_file(f"{build_name}.prj", prj_contents)
...
""".format(build_name=build_name, xst_opt=xst_opt, device=device)
    if vincpaths:
        ...
        for path in vincpaths:
            xst_contents += f"{tools.cygpath(path)} "
        xst_contents += "}"
    tools.write_to_file(f"{build_name}.xst", xst_contents)
...
def _run_yosys(device, sources, vincpaths, build_name):
    ...
    ys_contents = "".join(
        f"read_{language}{incflags} {filename}\n"
        for filename, language, library, *copy in sources
    )
...
"""
    ...
    if which("ise") is None and os.getenv("LITEX_ENV_ISE", False) == False:
        msg = (
            "Unable to find or source ISE toolchain, please either:\n"
            + "- Source ISE's settings manually.\n"
        )
        ...
        try:
            if mode in ["xst", "yosys", "cpld"]:
                ...
                if mode in ["xst", "cpld"]:
                    ...
                    isemode = "edif"
                    self.ngdbuild_opt += f"-p {platform.device}"
            ...
            if mode in ["edif"]:
                ...
                vns = e_output.ns
                named_sc, named_pc = platform.resolve_signals(vns)
                e_file = f"{build_name}.edif"
                e_output.write(e_file)
                isemode = "edif"

            # Generate design constraints (.ucf)
            tools.write_to_file(f"{build_name}.ucf", _build_ucf(named_sc, named_pc))
